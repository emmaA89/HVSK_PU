%-------------------------------------------------------------------------%
%
% File: RBF_PUM_diffmat_VSK_mat(epoints,dsites,h,h1,p)
%
% Goal: script that constructs VSKs differentiation matrices for the Matern
%       C^3 kernel
%
...
%
%-------------------------------------------------------------------------%
function [A,Ax,Axx,Ay,Ayy] = RBF_PUM_diffmat_VSK_mat(epoints,dsites,...
    h,h1,p)
N1 = size(epoints,1); N = size(dsites,1); % Initialize
[A,Ax,Ay,Axx,Ayy] = deal(zeros(N1,N));
% Generate differentiation matrices
if p == 1
    for i = 1:N1
        [psi,psix,psiy,psixx,psiyy] = mat_PUM_VSK(dsites,epoints(i,:),...
            h,h1,p);
...
if p == 0
    for i = 1:N1
        [psi] = mat_PUM_VSK(dsites,epoints(i,:),h,h1,p);
        A(i,:) = psi;
    end
