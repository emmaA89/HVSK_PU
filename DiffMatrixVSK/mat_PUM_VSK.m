%-------------------------------------------------------------------------%
%
% File: mat_PUM_VSK(dsites,epoints,h,h1,p)
%
% Goal: script that evaluates VSKs differentiation matrices for the Matern
%       C^3 kernel
%
...
%
%-------------------------------------------------------------------------%
function [w,wx,wy,wxx,wyy] = mat_PUM_VSK(dsites,epoints,h,h1,p)
...
if length(h) > 1
    ...
    if p == 0
        ...
        we = exp(-r).*(15+15*r+6*r.^2+r.^3); w = we';
    ...
    if p==1
        for i=1:size(dsites,1)
            ...
            dy = epoints(1,2) - dsites(i,2);
            r(i) = sqrt(dx.^2+dy.^2+(a1).^2);
            rx(i) = dx+a1*valx_ep; ry(i) = dy+a1*valy_ep;
            rxx(i) = valx_ep.^2+(a1).*valxx_ep;
            ryy(i) = valy_ep.^2+(a1).*valyy_ep;
        end
        % Compute the VSK differentiation matrices
        we = exp(-r).*(15+15*r+6*r.^2+r.^3); w=we';
        wex = -rx.*exp(-r).*(3+3*r+r.^2); wx=wex';
        wey = -ry.*exp(-r).*(3+3*r+r.^2); wy=wey';
        wexx =-exp(-r).*(1+rxx).*(3+3.*r+r.^2)+...
            exp(-r).*(rx).^2.*(r+1);  wxx=wexx';
        weyy =-exp(-r).*(1+ryy).*(3+3.*r+r.^2)+...
            exp(-r).*(ry).^2.*(r+1);  wyy=weyy';
    end
else
    if p==0
        % Evaluate the scale function
    ...
    if p==1
        ...
        rx = dx+a1*valx_ep;
        ry = dy+a1*valy_ep;
        rxx = valx_ep.^2+(a1).*valxx_ep;
        ryy = valy_ep.^2+(a1).*valyy_ep;
        % Computes the VSK differentiation matrices
        we = exp(-r).*(15+15*r+6*r.^2+r.^3);  w=we';
        wex = -rx.*exp(-r).*(3+3*r+r.^2); wx=wex';
        wey = -ry.*exp(-r).*(3+3*r+r.^2); wy=wey';
        wexx = -exp(-r).*(1+rxx).*(3+3.*r+r.^2)+...
